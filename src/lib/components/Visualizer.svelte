<script lang="ts">
    ...
    function startVisualizer() {
        ...
        function draw() {
            ...
            for (let y = horizonY; y < height; y += 40 * ((y / horizonY) * 0.5)) {
                // Creating a pseudo-perspective effect where lines get closer together near horizon is tricky in 2D
                // So we just move linear lines but mask them or space them
                // Let's do a simple moving set of horizontal lines
                // We'll draw them based on a perspective formula: y = horizonY + distance^2
            }
